        ...
        <div className='flex flex-col md:flex-row justify-between items-center w-full' >
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
                {exclusiveOffers.map((item)=>(
                    <div key={item.id} className='group relative flex flex-col item-start jusify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
                        <p className='px-3 py-1 absolute top-4 left-4 text-xs bg-white text-black font-medium rounded-full'>{item.priceOff}% OFF</p>
                        <div>
